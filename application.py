...
def result():
    req = request.form
    result = imdScore.score(float(req['stock-price']),float(req['4-years']),float(req['book-value']),float(req['buy-analysts']),float(req['strong-buy']),float(req['yield']),float(req['margin']),float(req['volume']),float(req['pe']))
    print(result)
